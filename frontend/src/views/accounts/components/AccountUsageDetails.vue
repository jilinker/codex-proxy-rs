...
const props = defineProps<{
  usage: AccountUsagePresentation
  showBilling?: boolean
}>()

type AccountModelUsage = AccountUsagePresentation['models'][number]

const totalBilling = computed(() => props.usage.costs?.find(cost => cost.currency.toUpperCase() === 'USD'))
const totalBillingDisplay = computed(() => totalBilling.value?.estimatedAmountDisplay ?? '—')
const hasUsageSummary = computed(() => (props.usage.totalTokens ?? 0) > 0 || totalBilling.value !== undefined)

const modelUsageColumns = computed(() => defineTableColumns<AccountModelUsage>([
  ...
  ...(props.showBilling ? [{ key: 'billingAmountUsdDisplay', label: '计费', kind: 'numeric' as const, size: 'sm' as const }] : []),
  ...
  <section class="grid gap-4 rounded-lg bg-cp-bg-container p-4 shadow-cp-tertiary xl:min-h-0 xl:grid-cols-[0.52fr_1.48fr]">
    <div class="xl:flex xl:min-h-0 xl:flex-col">
      <div class="mb-3 flex shrink-0 items-center justify-between gap-3">
        ...
        <span class="text-cp-xs font-emphasis text-cp-text-quaternary">{{ usage.windowLabelDisplay }}</span>
      </div>
    ...
    <div class="min-w-0 pt-4 xl:flex xl:min-h-0 xl:flex-col xl:pt-0 xl:pl-4">
      <div class="mb-3 flex shrink-0 flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <div class="ml-auto flex items-baseline gap-4">
          <div v-if="hasUsageSummary" class="flex items-baseline gap-1.5 whitespace-nowrap">
            ...
            <span title="总 Token">
              ...
              <span class="font-mono text-cp-sm font-emphasis tabular-nums text-cp-text">
                {{ usage.totalTokensDisplay }}
              </span>
            </span>
            <span v-if="showBilling" class="mx-0.5 text-[10px] leading-none font-emphasis text-cp-text-quaternary"> / </span>
            <span v-if="showBilling" title="总计费">
          ...
          <span class="whitespace-nowrap text-cp-xs font-emphasis text-cp-text-quaternary">{{ usage.windowLabelDisplay }}</span>
        </div>
      </div>
